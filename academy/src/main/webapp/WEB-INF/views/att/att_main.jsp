...
$(document).ready(function(){
	$('.list').each(function(index){
		$(this).attr('menu-index',index);
			...
			$('.list[menu-index='+index+']').addClass('selected_list');
			$('.list[menu-index!='+index+']').removeClass('selected_list');
		});
...
function att_Yesterday(){
	...
	var dateInfo=dat.getFullYear()+'-'+month+'-'+date+'('+day+')';
	location.href='att_main.do?dateInfo='+dateInfo;
}
		...
		if(XHR.status==200){			
			...
			if (datas==''){
				...
				for(i=0; i<datas.length;i++){				
					...
					result+='<tr class="list" onclick="selectStudent(\''+data.ST_IDX+'\',\''+data.ST_NAME+'\',\''+data.ATT_DIVIDE+'\',\''+data.ATT_BIGO+'\',\''+i+'\',\''+datas.length+'\')"><td>'+data.ST_NAME+'</td><td>'+data.ATT_DIVIDE+'</td><td>'+data.ATT_BIGO+'</td></tr>';
				}
